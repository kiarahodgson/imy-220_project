...
router.post('/:userId/unfriend', async (req, res) => {
    ...
    console.log("Unfriend request received:", { userId, friendId });
    ...
    try {
        ...
        // Check if users are actually friends before removing
        if (!user.friends.includes(friendId) || !friend.friends.includes(userId)) {
            console.error("Users are not friends, cannot proceed with unfriend operation");
            return res.status(400).json({ message: 'Users are not friends' });
        }

        // Log the friend lists before modification
        console.log("User's friends before removal:", user.friends);
        console.log("Friend's friends before removal:", friend.friends);
        ...
        console.log("Successfully unfriended:", { userId, friendId });
