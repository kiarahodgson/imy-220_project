...
const ProfilePage = ({ userId, loggedInUserId, comments = [] }) => {
  ...
  const handleUnfriend = async () => {
    ...
    try {
        const response = await fetch(`http://localhost:8000/api/users/${loggedInUserId}/unfriend`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        // Update profile data to remove friend from the list
        setProfileData((prevData) => {
            console.log("Current friends list before removal:", prevData.friends);
            const updatedFriends = prevData?.friends?.filter(friend => friend._id !== userId) || [];
            console.log("Updated friends list after removal:", updatedFriends);
            return { ...prevData, friends: updatedFriends };
        });

        setFriendRequestStatus(''); // Reset friend status
    } catch (error) {
        console.error("Error in handleUnfriend function:", error);
    }
};
        ...
        <section className="profile-info">
          ...
          {isViewingOwnProfile || friendRequestStatus === 'friends' ? (
            <>
              ...
              {friendRequestStatus === 'friends' && !isViewingOwnProfile && (
                <button onClick={handleUnfriend} className="off-white-button bg-red-500 text-white mt-2 ml-2">
                  Unfriend
                </button>
              )}
            </>
